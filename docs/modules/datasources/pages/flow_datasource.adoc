...
=== tosOnExporterNodeAndInterface

The `tosOnExporterNodeAndInterface()` query will return a list of ToS values which are used by one or more flows on the given node and interface.
The value of the template variable is the `ToS Value`.
...
```
tosOnExporterNodeAndInterface($node, $interface)
```
...
The value of the template variable is the `DSCP Value` and the display text is the `DSCP Name` (if valid).
...
Assuming that the `$node` variable is populated with a value from the `exporterNodesWithFlows` query and that the `$interface` variable is populated with a value from the `interfacesOnExporterNodeWithFlows($node)` query, you can use the following expression:
```
dscpOnExporterNodeAndInterface($node, $interface)
```

=== ecn

The `ecn()` will return a list of ECN values.
The value of the template variable is the `ECN Value` and the display text is the `ECN Name` (if valid).

You can use the following expression to query for valid ECN values:
```
ecn()
```
