...
A filter panel allows you to configure a series of variables to put in a dashboard. 
While xref:pc-template-filters[template variables] work with the performance datasource, the filter panel coordinates with any panels in the dashboard that are using the xref:../datasources/entity_datasource.adoc#[entity datasource], to automatically apply filters that match those variables.

To create a filter panel, follow these steps:

. Select or xref:../getting_started/basic_walkthrough.adoc#[create a panel] that uses an entity datasource. 
